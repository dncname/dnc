...
def run(args):
    ...
    if subprocess.call(args, shell=True):
        print('bios-boot-dnc.py: exiting because of error')
        sys.exit(1)

def retry(args):
    while True:
        print('bios-boot-dnc.py:', args)
        logFile.write(args + '\n')
        if subprocess.call(args, shell=True):
            print('*** Retry')
        else:
...
def addB1Account():
    run(args.cleos + 'create account dnc b1 ' + datas["initAccounts"][len(datas["initAccounts"]) - 1]['key'] + " -p dnc")

def addRelayAccount():
    for a in relayAccounts:
        run(args.cleos + 'create account dnc ' + a + ' ' + relayPubKey)
        run(args.cleos + 'push action eosio transfer \'{"from":"dnc","to":"%s","quantity":"100000.0000 DNC","memo":""}\' -p dnc' % a)
...
def importKeys():
    ...
    for a in datas["initAccountsKeys"]:
        ...
        if not key in keys:
            keys[key] = True
            # note : new develop dnc change this command to wallet import--private-key pk
...
parser.add_argument('--root', metavar='', help="dnc root dir from git", default='../../')
...
if not haveCommand:
    print('bios-boot-dnc.py: Tell me what to do. -a does almost everything. -h shows options.')
