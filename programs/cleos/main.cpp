...
authority parse_json_authority_or_key(const std::string& authorityJsonOrFile) {
   if (boost::istarts_with(authorityJsonOrFile, "DNC") || boost::istarts_with(authorityJsonOrFile, "PUB_R1")) {
      try {
...
struct vote_producer_vote_subcommand {
   ...
   vote_producer_vote_subcommand(CLI::App* actionRoot) {
      ...
      vote_proxy->add_option("amount", amount, localized("The amount of DNC to vote"))->required();
      add_standard_transaction_options(vote_proxy);
      
      vote_proxy->set_callback([this] {
          char doubleamount[20] = {0};
          sprintf(doubleamount,"%.4f",amount);
          auto vote_amount = string(doubleamount) + " DNC";
         fc::variant act_payload = fc::mutable_variant_object()
                  ("voter", voter_str)
                  ("bpname", bpname_str)
