...
BOOST_FIXTURE_TEST_CASE( delay_create_account, validating_tester) { try {
   ...
   account_name creator = N(dnc)/*config::system_account_name*/;
...
const std::string eosio_token = name(N(dnc)/*N(eosio.token)*/).to_string();
#if 0
...
BOOST_AUTO_TEST_CASE(delete_auth_test) { try {
   ...
   // create CUR token
   chain.produce_blocks();
   chain.push_action(N(eosio.token), N(create), N(dnc)/*N(eosio.token)*/, mutable_variant_object()
           ("issuer", "eosio.token" )
           ("maximum_supply", "9000000.0000 CUR" )
   );

   // issue to account "eosio.token"
   chain.push_action(N(eosio.token), name("issue"), N(dnc)/*N(eosio.token)*/, fc::mutable_variant_object()
           ("to",       "eosio.token")
           ("quantity", "1000000.0000 CUR")
           ("memo", "for stuff")
   );
   ...
   trace = chain.push_action(N(eosio.token), name("transfer"), N(dnc)/*N(eosio.token)*/, fc::mutable_variant_object()
       ("from", "eosio.token")
       ("to", "tester")
       ("quantity", "100.0000 CUR")
       ("memo", "hi" )
   ...
   liquid_balance = get_currency_balance(chain, N(dnc)/*N(eosio.token)*/);
   BOOST_REQUIRE_EQUAL(asset::from_string("999900.0000 CUR"), liquid_balance);
   liquid_balance = get_currency_balance(chain, N(tester));
   BOOST_REQUIRE_EQUAL(asset::from_string("99.0000 CUR"), liquid_balance);
   liquid_balance = get_currency_balance(chain, N(tester2));
...
BOOST_AUTO_TEST_CASE( mindelay_test ) { try {
   ...
   auto trace = chain.push_action(N(eosio.token), name("transfer"), N(dnc)/*N(eosio.token)*/, fc::mutable_variant_object()
       ("from", eosio_token)
       ("to", "tester")
       ("quantity", "100.0000 CUR")
       ("memo", "hi" )
   ...
   auto liquid_balance = get_currency_balance(chain, N(dnc)/*N(eosio.token)*/);
   BOOST_REQUIRE_EQUAL(asset::from_string("999900.0000 CUR"), liquid_balance);
   liquid_balance = get_currency_balance(chain, N(tester));
   BOOST_REQUIRE_EQUAL(asset::from_string("100.0000 CUR"), liquid_balance);
...
BOOST_AUTO_TEST_CASE( canceldelay_test ) { try {
   ...
   //chainbase::database db(const_cast<chainbase::database &>(chain.control->db()));
   //set_num_config_on_chain(db, config::func_typ::use_system01, 2);
   
   chain.push_action(config::system_account_name, setconfig::get_name(), config::chain_config_name, fc::mutable_variant_object()
           ("typ", name(config::func_typ::use_system01).to_string())
           ("num", "2")
           ("account_name", "active")
           ("fee",  "0.0001 DNC")
   );
   ...
   chain.create_account(N(tester));
   chain.create_account(N(tester2));
   //chain.produce_blocks(chain.vcfg.System01_contract_block_num/*10*/);
...
BOOST_AUTO_TEST_CASE( canceldelay_test2 ) { try {
   ...
   chain.produce_blocks();
   auto liquid_balance = get_currency_balance(chain, eosio_token/*N(eosio.token)*/);
   BOOST_REQUIRE_EQUAL(asset::from_string("999900.0000 CUR"), liquid_balance);
   liquid_balance = get_currency_balance(chain, N(tester));
   BOOST_REQUIRE_EQUAL(asset::from_string("100.0000 CUR"), liquid_balance);
...
BOOST_FIXTURE_TEST_CASE( delay_expired, validating_tester) { try {
   ...
   set_transaction_headers(trx);
   trx.delay_sec = 3;
   trx.expiration = control->head_block_time() + fc::microseconds(4500000/*1000000*/);
   trx.sign( get_private_key( creator, "active" ), control->get_chain_id()  );
