...
BOOST_FIXTURE_TEST_CASE(eosio_abi, TESTER) try {
   ...
   trx.actions.emplace_back( vector<permission_level>{{N(dnc)/*config::system_account_name*/,config::active_name}},
                             newaccount{
                                   .creator  = N(dnc)/*config::system_account_name*/,
                                   .name     = a,
                                   .owner    = owner_auth,
                                   .active   = authority( get_public_key( a, "active" ) )
                             });
   set_transaction_headers(trx);
   trx.sign( get_private_key( N(dnc)/*config::system_account_name*/, "active" ), control->get_chain_id()  );
   auto result = push_transaction( trx );
