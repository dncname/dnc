...
class currency_tester : public TESTER {
   public:
      ...
      auto issue(const account_name& to, const std::string& quantity, const std::string& memo = "") {
         auto trace = push_action(N(dnc)/*N(eosio.token)*/, N(issue), mutable_variant_object()
                                  ("to",       to)
                                  ("quantity", quantity)
                                  ("memo",     memo)
                                  );
      ...
      :TESTER(),abi_ser(json::from_string(eosio_token_abi).as<abi_def>(), abi_serializer_max_time)
      {
         ...
         auto result = push_action(N(dnc)/*N(eosio.token)*/, N(create), mutable_variant_object()
                 ("issuer",       eosio_token)
         ...
         result = push_action(N(dnc)/*N(eosio.token)*/, N(issue), mutable_variant_object()
                 ("to",       eosio_token)
...
const std::string currency_tester::eosio_token = name(N(dnc)/*N(eosio.token)*/).to_string();
...
BOOST_AUTO_TEST_CASE( bootstrap ) try {
   ...
   currency_tester t;
   auto actual = t.get_currency_balance(N(eosio.token), expected.get_symbol(), N(dnc)/*N(eosio.token)*/);
...
BOOST_FIXTURE_TEST_CASE( test_transfer, currency_tester ) try {
   ...
   {
      auto trace = push_action(N(dnc)/*N(eosio.token)*/, N(transfer), mutable_variant_object()
         ("from", eosio_token)
         ("to",   "alice")
...
BOOST_FIXTURE_TEST_CASE( test_duplicate_transfer, currency_tester ) {
   ...
   BOOST_REQUIRE_THROW(push_action(N(dnc)/*N(eosio.token)*/, N(transfer), mutable_variant_object()
                                    ("from", eosio_token)
                                    ("to",   "alice")
                                    ("quantity", "100.0000 CUR")
                                    ("memo", "fund Alice")),
