...
void apply_eosio_setfee(apply_context& context) {
   ...
   * support 1.0 DNC is for `C` cpu, `N` net and `R` ram,
   * and cost fee by `f` DNC and extfee by `F` DNC
   *
   * then can give:
   ...
   * */
   ...
   EOS_ASSERT(act.fee.get_amount() <= (200 * 10000),
         invalid_action_args_exception, "fee can not too mush, more then 200.0000 DNC");
